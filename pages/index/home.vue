<template>
	<view class="content">
		...
		<!-- 头部区域 -->
		<view class="header">
			<view class="user" v-show="false"><i class="icon icon-morentouxiang"></i></view>
			<view class="search" @tap="goToSearch(searchHotCurrent)">
				<i class="icon icon-sousuo"></i>
				<swiper class="search-word" circular :autoplay="true" :interval="4000" :vertical="true"
					@change="changeSearch">
			...
			<swiper class="swiper" circular :indicator-dots="true" :autoplay="true" :interval="4000"
				...
				<swiper-item class="swiper-item" v-for="(item, index) in bannerList" :key="index">
					<view class="banner-item">
						<image class="swiper-img" :src="item.imageUrl" mode="widthFix"></image>
					</view>
				</swiper-item>
			</swiper>
		</view>
		...
		<!-- 推荐歌单 -->
		<songs-list title="推荐歌单" more="更多" :dataList="recSongList"></songs-list>
		<!-- 最新专辑 -->
		<songs-list title="最新专辑" more="更多" :dataList="albumNewest"></songs-list>
		<!-- 排行榜 -->
		<top-detail title="排行榜" more="更多" :dataList="topList"></top-detail>
		<!-- 最新音乐 -->
		<songs-list title="最新音乐" more="更多" :dataList="newSongList"></songs-list>
	</view>
</template>
...
<script>
	...
	export default {
		...
		methods: {
			...
			goToSearch(keywords) {
				// #ifdef H5
				const items = encodeURIComponent(JSON.stringify(this.searchHotCurrent));
				// #endif
				// #ifdef MP-QQ||MP-WEIXIN
				const items = JSON.stringify(this.topList);
				// #endif
				wx.navigateTo({
					url: `/pages/search/search?keywords=${keywords}&searchHotCurrent=${items}`
				})
			},
		}
	}
